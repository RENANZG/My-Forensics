...
# USAGE:
#  Make it executable:
#   $ sudo chmod +x ~/Downloads/Blensor_AppImage.sh
#  Then run
#   $ sudo bash ~/Downloads/Blensor_AppImage.sh
########################################################################
...
version=$(curl -Ls https://www.blensor.org/pages/downloads.html | grep -Eo "(http|https)://[a-zA-Z0-9./?=_%:-]*" | sort -u | grep -i appimage)
wget $version
